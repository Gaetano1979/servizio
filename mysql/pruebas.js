...
let update = (modifica, callback) => {
    ...
    mysql.conessione.query(`UPDATE clientes SET cliente=?,ciudad=?,zona=?,direccion=?,telefono1=?,celular=?,email=? where idcliente=?`, [modifica[0], modifica[1], modifica[2], modifica[3], modifica[4], modifica[5], modifica[6]], modifica[7], (err, result) => {
        if (err) {
            return callback('Error ', err);
        } else {
